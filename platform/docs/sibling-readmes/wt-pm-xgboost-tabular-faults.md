...
Gradient-boosted trees for real-time multi-class component-state classification on structured sensor features.

This repository stays independently usable (`python model.py`). The unified
**wt-pm** platform wraps the original `model.py` through adapter `m11-xgboost-tabular` —
...
| | |
|---|---|
| Input (adapter view) | tabular features + labels (research) |
| Output (WTDataSchema) | prediction, probability |
| Integration role | Primary tabular voter. SHAP TreeExplainer source (m21). Hermes `explain`. |
...
wt-pm inspect          # see m11-xgboost-tabular availability
