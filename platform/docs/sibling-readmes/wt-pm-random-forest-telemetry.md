...
**wt-pm** platform wraps the original `model.py` through adapter `m10-random-forest` —
...
| Output (WTDataSchema) | prediction, probability, importances |
| Integration role | Tabular voter + XAI evidence. Hermes `diagnose` / `explain`. |
...
wt-pm inspect          # see m10-random-forest availability
