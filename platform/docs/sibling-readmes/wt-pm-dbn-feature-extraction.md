...
Deep Belief Network of stacked RBMs building an automated feature hierarchy for wear tracking.

This repository stays independently usable (`python model.py`). The unified
**wt-pm** platform wraps the original `model.py` through adapter `m09-dbn-features` —
...
| Integration role | Feature provider. Platform supplies CD-1 pretraining the repo omits. |
...
wt-pm inspect          # see m09-dbn-features availability
