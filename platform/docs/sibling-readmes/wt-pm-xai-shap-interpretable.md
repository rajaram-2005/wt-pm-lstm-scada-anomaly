...
> **Category:** Physics-Informed / Explainable AI  
> **Platform adapter:** `m21-xai-shap`  
> **Status:** integrated

## Description

SHAP (+ LIME in upstream) interpretation wrapper providing root-cause diagnostic explanations for black-box alerts.

This repository stays independently usable (`python model.py`). The unified
**wt-pm** platform wraps the original `model.py` through adapter `m21-xai-shap` —
...
| | |
|---|---|
| Input (adapter view) | fitted tree model + feature matrix |
| Output (WTDataSchema) | top-k attributions |
| Integration role | Hermes `explain` tool. Platform adds contrastive healthy-band z and a feature counterfactual on top of TreeExplainer. |
...
wt-pm inspect          # see m21-xai-shap availability
