...
**wt-pm** platform wraps the original `model.py` through adapter `m14-isolation-forest` —
...
| | |
|---|---|
| Input (adapter view) | tabular features (mask-filtered; adapter refuses fillna(0)) |
| Output (WTDataSchema) | anomaly_score |
| Integration role | Universal fallback for every deep anomaly model. Hermes `anomaly`. |
...
wt-pm inspect          # see m14-isolation-forest availability
