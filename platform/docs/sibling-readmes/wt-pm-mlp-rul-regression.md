...
**wt-pm** platform wraps the original `model.py` through adapter `m17-mlp-rul` —
...
| | |
|---|---|
| Input (adapter view) | tabular features + proxy RUL target (stated as proxy) |
| Output (WTDataSchema) | rul_hours |
| Integration role | Baseline every other RUL model must beat. Hermes `rul`. |
...
wt-pm inspect          # see m17-mlp-rul availability
