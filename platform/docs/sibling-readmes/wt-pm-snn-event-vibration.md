...
**wt-pm** platform wraps the original `model.py` through adapter `m07-snn-vibration` —
...
| | |
|---|---|
| Input (adapter view) | delta-encoded vibration surrogates |
| Output (WTDataSchema) | prediction, probability (spike-rate) |
| Integration role | Low-power drivetrain voter on CLOUD and EDGE. Hermes `diagnose`. |
...
wt-pm inspect          # see m07-snn-vibration availability
