...
Deep neural surrogate replacing heavy FEA for real-time digital-twin residual tracking.

This repository stays independently usable (`python model.py`). The unified
**wt-pm** platform wraps the original `model.py` through adapter `m19-digital-twin` —
...
| | |
|---|---|
| Input (adapter view) | operating-condition channels |
| Output (WTDataSchema) | stress/deflection/fatigue proxies |
| Integration role | Hermes `what_if` tool (derate scenarios). Physics-proxy targets, labelled as such. |
...
wt-pm inspect          # see m19-digital-twin availability
