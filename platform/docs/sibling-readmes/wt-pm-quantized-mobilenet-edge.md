...
> **Status:** integrated (recipe)

## Description

Quantized MobileNet variant for compressed high-frequency feature extraction on edge hardware.

This repository stays independently usable (`python model.py`). The unified
**wt-pm** platform wraps the original `model.py` through adapter `m24-quantized-edge` —
...
| | |
|---|---|
| Input (adapter view) | a Keras model + representative data |
| Output (WTDataSchema) | INT8 .tflite artifact |
| Integration role | Conversion recipe only. Full 224×224 MobileNet is out of scope (no image data in the collection). Applied to a small telemetry edge net (~3.4 KB). |
...
wt-pm inspect          # see m24-quantized-edge availability
